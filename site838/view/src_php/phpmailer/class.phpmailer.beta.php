...
//public:
    ...
    /////////////////////////////////////////////////
//private:
	var $arr_smtp_ok = array(); //smtp发送成功邮件array(mail=>true)
    /**
    ...
    */
    ...
    function smtp_send($header, $body) {
		...
		//echo "smtp_send start<br/>\n";
        include_once($this->PluginDir . "class.smtp.beta.php");
	...
	//exit("send mail smtp:".$port.":".$this->Timeout);
			//$this->Timeout = 5;
            if($smtp->Connect($host, $port, $this->Timeout))
                $connection = true;
            //var_dump(count($hosts));
			//exit();
	...
	if ($this->Helo != '')
        ...
        //var_dump($this->get_server_hostname());
		//exit();
        ...
        {
			//echo "Recipient to:".$this->to[$i][0]."<br/>\n";
			if(!$smtp->Recipient(sprintf("<%s>", $this->to[$i][0])))
                $bad_rcpt[] = $this->to[$i][0];
			else
				$this->arr_smtp_ok[$this->to[$i][0]] = true;
        }
        for($i = 0; $i < count($this->cc); $i++)
        {
            if(!$smtp->Recipient(sprintf("<%s>", $this->cc[$i][0])))
				...
				$this->arr_smtp_ok[$this->cc[$i][0]] = true;
        }
        for($i = 0; $i < count($this->bcc); $i++)
        {
			//if($i > 1)
			//	break;
			//echo "Recipient bcc to:".$this->bcc[$i][0]."<br/>\n";
            if(!$smtp->Recipient(sprintf("<%s>", $this->bcc[$i][0])))
                $bad_rcpt[] = $this->bcc[$i][0];
			else
				$this->arr_smtp_ok[$this->bcc[$i][0]] = true;
        }
		//echo "Recipient over,num=".count($this->arr_smtp_ok)."<br/>\n";
		/*
