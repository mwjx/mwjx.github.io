...
///usr/local/php/bin/php /usr/home/mwjx/fish838.com/site838/view/track/novels_info.php
//var_dump(extension_loaded("zlib")); 
exit();
$_SERVER["PHP_SELF"] = "/site838/view/track/novels_info.php";
...
//-----end tests------
$m_author = array(); //作者=>ID
$m_novels = array(); //作品=>ID
$m_links = array(); //作品ID=>来源ID=>true
load_novels();
...
function load_novels()
{
	...
	for($i = 0;$i < $len; ++$i){
		$id = intval($arr[$i][0]);
		$name = ($arr[$i][1]);
		if(isset($m_author[$name]))
			continue;
		$m_author[$name] = $id;
	}
	//作品
	$str_sql = "select id,title from novels;";
	$sql = new mysql("fish838");
	$sql->query($str_sql);
	$sql->close();
	$arr = $sql->get_array_array();
	...
	//链接
	$str_sql = "select novels,sou from novels_links;";
	//exit($str_sql);
	$sql = new mysql("fish838");
	$sql->query($str_sql);
	$sql->close();
...
function deal_line($line="",&$out,$site=-1)
{
	...
	//exit("|".$author."|");
	$out = array(trim($title),trim($author),$nid,$s);
	return;
	//site:4
